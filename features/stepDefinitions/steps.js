...
When('save is called on the model', function(){
  return this.modelInstance.functions.save().then(x=>
    this.saveResult = x
  )
})

When('delete is called on the model', function(){
  return this.modelInstance.functions.delete().then(x=>
    this.deleteResult = x
  )
})
...
When("the datastore's retrieve is called with values", function(table) {
  ...
  const tableName = rows.tableName
  const modelName = rows.modelName
  const id = rows.id
  return this.datastoreProvider.retrieve(tableName, modelName, id).then(obj => {
    this.result = obj
  })
})
